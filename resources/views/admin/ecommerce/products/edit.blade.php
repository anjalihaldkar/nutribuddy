                ...
                <div class="card bg-white border-0 shadow-lg sticky-bottom py-20 px-32 radius-16 mb-40">
                    <div class="d-flex justify-content-end gap-16">
                        ...
                        <button type="submit" class="btn btn-primary-600 px-40 fw-bold">
                            <iconify-icon icon="lucide:save" class="me-8"></iconify-icon> UPDATE PRODUCT
                        </button>
                    </div>
                </div>
            </form>
        ...
        @if(!empty($currentTags) && is_array($currentTags))
            @foreach($currentTags as $tag)
                @php
                    $isEmoji = false;
                    $icon = $tag['icon'] ?? '';
                    if ($icon && preg_match('/^[\x{1F300}-\x{1F9FF}\x{2600}-\x{26FF}\x{2700}-\x{27BF}]$/u', $icon)) $isEmoji = true;
                @endphp
                addTagRow('{{ $tag['icon'] ?? "" }}', '{{ addslashes($tag['text'] ?? "") }}', {{ $isEmoji ? 'true' : 'false' }});
            @endforeach
        @endif
